...
- ✅ **公開角色市場 M04**（2026/05/22 完成）：用戶投稿角色（三步驟：選角色→多選圖片→確認），管理員後台審核，核准後自動上架到首頁瀑布流和 /explore 探索頁，退件附原因通知用戶。
...
✅ VoiceSelector 元件統一 + Voice Cloning 功能（2026/05/24）：三個入口（SaveCharacterModal/characters頁/create頁Upload Modal/TtsModal）統一使用 VoiceSelector 元件，付費用戶可上傳本人聲音克隆，內建錄音功能，克隆聲音試聽不受次數限制，選克隆時隱藏下載語音按鈕。
✅ 聊天室停聊 60 秒後角色主動發話（單人+群組）
✅ Vercel 升級 Pro（$20 USD/月）（2026/05/05）
✅ 每日簽到可兌換額外1次影片生成次數（免費用戶專屬，簽到後當日影片額度從1支→2支，不累積到隔天）
✅ 每日簽到改為幸運轉盤（2026/05/10）：隨機獲得1-5點，機率：1點65%/2點25%/3點5%/4點4%/5點1%，平均期望值約1.41點，連續獎勵不變
...
✅ 首頁未登入可見瀑布流（2026/05/24）：未登入用戶可瀏覽首頁畫廊（前12張），點聊天或第12張後觸發登入提示，引導 Google 登入取得免費5點。

---

## 未來功能規劃
→ 詳見 DNA_ROADMAP.md（L 系列 + 待辦雜項）
