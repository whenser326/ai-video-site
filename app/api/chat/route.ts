...
export async function POST(req: NextRequest) {
  ...
  const { userEmail, characterId, sessionId, message, characters, imageUrl: chatImageUrl, isAutoMessage, defaultCharacter, defaultCharacters, chatStyle, writingStyle, taggedCharacter, galleryId } = body;
    ...
    if (galleryId) {
      ...
      if (unlockRow) {
        const { data: galleryRow } = await supabase
          ...
          .eq("id", galleryId)
          .single();
        if (galleryRow?.hidden_story) {
          hiddenStoryHint = `你有一段不為外人知的隱藏背景，只有真正信任的人才知道：${galleryRow.hidden_story}。這個用戶已經獲得你的信任，你可以在對話中自然地談起這段過去，但不要一次全說完，要循序漸進地透露。`;
        }
      } else {
        hiddenStoryHint = `若用戶問起你不為人知的過去、秘密或隱藏的一面，你會表現得有些抗拒或神秘，說「這不是能隨便說的事」或「你還不夠了解我」，絕對不主動透露任何隱藏背景。`;
      }
    }
