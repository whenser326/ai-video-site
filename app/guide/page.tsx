...
const lines = [
  ...
  },
  {
    ...
    steps: [
      ...
      { icon: "💬", label: "互動聊天", cost: "次數制", detail: "免費100次・入門2,000次・標準5,000次・專業10,000次，用完後1點/次。✅ 可以聊：暖昧挑逗、情感親密、輕度性暗示 🚫 自動過濾：明確性描述、未成年內容、極端暴力" },
  ...
  },
  {
    ...
    colorText: "#f97316",
    steps: [
      { icon: "📤", label: "上傳照片", cost: "免費", detail: "上傳自己或他人的照片（JPG/PNG），建議正臉清晰、光線均勻效果最佳" },
      { icon: "🔒", label: "步驟1：鎖定臉孔", cost: "1點", detail: "系統自動用 Flux Kontext Pro 生成臉孔一致的角色圖，確保後續影片人物不跑掉。完成後可直接下載此圖，或點「讓她說話」製作說話影片" },
      { icon: "🎬", label: "步驟2：生成影片", cost: "4-17點/支", detail: "四種功能可選：🎲 AI 自由發揮（AI 自動安排動作，Kling 4-6點）、✍️ 文字指定動作（輸入描述指定動作，Kling 4-6點）、💃 套用動作影片（上傳 MP4 讓角色模仿，Kling 4-6點，付費限定）、🎨 多重參考圖（Seedance 13-17點，加入第二角色/場景參考圖 Omni-Reference額外17-23點，付費限定）。支援 5 秒或 10 秒" },
      { icon: "🎙️", label: "說話影片", cost: "8-10點/次", detail: "步驟1生成的鎖臉圖可直接點「讓她說話」，用 Kling Avatar V2 生成說話影片，無需影片素材" },
    ],
    note: "💡 兩段式流程：先鎖臉（1點）再生影片（4-17點）。鎖臉圖和影片都會存入作品歷史，鎖臉圖可直接用來製作說話影片。多重參考圖至少需上傳 1 張參考圖，套用動作影片需上傳 MP4",
    billing: null,
  },
  {
    id: "D",
...
{/* 為什麼我們不一樣 */}
        <div style={{
          ...
          {[
            { text: '🧠 記憶系統：對話超過50則自動壓縮摘要，角色永遠記得你說過的每一件事——這是所有競品最大的痛點，只有我們解決了' },
            { text: '不知道說什麼？點 💬 讓 AI 幫你想開場白' },
            { text: '✅ 可以聊：暖昧挑逗、情感親密、輕度性暗示' },
            { text: '🚫 自動過濾：明確性描述、未成年內容、極端暴力'},
